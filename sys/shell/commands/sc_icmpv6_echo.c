...
int _icmpv6_ping(int argc, char **argv)
{
    ...
    ping_start = xtimer_usec_from_ticks64(xtimer_now64());
    ...
    while ((remaining--) > 0) {
        ...
        start = xtimer_usec_from_ticks(xtimer_now());
        ...
        if (xtimer_msg_receive_timeout64(&msg, (uint64_t)timeout) >= 0) {
            switch (msg.type) {
                case GNRC_NETAPI_MSG_TYPE_RCV:
                    stop = xtimer_usec_from_ticks(xtimer_now()) - start;
        ...
        if ((++stat_counter == stat_interval) || (remaining == 0)) {
            uint64_t total_time = xtimer_usec_from_ticks64(xtimer_now64()) - ping_start;
